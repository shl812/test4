...
int userin(char *p) { // “>” 이후에 적는 프롬프트 프린트하고 읽는 함수
	int c, count;
	...
	while (1) {
		if ((c = getchar()) == EOF) // 한 줄 입력 받음
			return EOF;

		if (count < MAXBUF)
			inpbuf[count++] = c; // inpbuf에 단어를 하나씩 대입

		if (c == '\n' && count < MAXBUF) {
			inpbuf[count] = '\0'; // 문장의 끝에 null을 넣음
			return count;
		}

		if (c == '\n') { // count == MAXBUF가 되면 재시작
...
int gettok(char **outptr) {  // 토큰을 가져와서 tokbuf에 넣는 함수
	int type;

	*outptr = tok; // outptr이 가리키는 문자열을 tok으로 함

	while (*ptr == ' ' || *ptr == '\t') // 공백 제거
		ptr++;

	*tok++ = *ptr; // 토큰 포인터(*tok)를 버퍼내 첫 토큰으로 지정

	switch (*ptr++) { // 토큰에 따라 유형 변수를 지정
	case '\n':
		type = EOL;
		break;
	case '&':
...
int inarg(char c) { // 유효한 보통문자들을 계속 읽어들이는 함수
	char *wrk;
...
int procline(void) { // 입력줄 처리
	...
	while (1) { // 무한루프
		...
		case SEMICOLON:
		case AMPERSAND:
			if (toktype == AMPERSAND) // '&'이면 백그라운드 or 포어그라운드
				type1 = BACKGROUND;
			else
				type1 = FOREGROUND;
			...
			if (toktype == EOL) 
				return 0;		// 한 줄이 끝나면 0 리턴
			narg = 0;
			break;
		case PIPE:
			type2 = toktype;	// type2 : 파이프타입
			if (narg < MAXARG)	
				narg++;
			break;
		}
...
int runcommand(char **cline, int where, int pipe) { // 명령 수행
	...
	if (strcmp(cline[0], "cd") == 0) { // cd 명령어
		if (chdir(cline[1]) == -1) { // 디렉토리 바꿈
	...
	case -1:
		...
		return -1;
	case 0: // 자식코드
		if (where == BACKGROUND) { // 백그라운드에서는 시그널 무시
			act.sa_handler = SIG_IGN;
			sigaction(SIGINT, &act, NULL);
			sigaction(SIGQUIT, &act, NULL);
		} else { // 포어그라운드에서 시그널 효력 생김
	...
	// 부모코드
	if (where == BACKGROUND) { // 백그라운드면 식별자 출력
		printf("[Process id %d]\n", pid);
		return 0;
	}
...
int join(char *com1[], char *com2[]) { // 파이프 실행
	...
	default:
		...
		if (tmp2[0] == 0) { // 마지막 명령어가 없으면 파이프 종료
			execvp(com2[0], com2);
...
void sepstring(char *com1[], char *com2[], char *str[]) {
	...
	for (int i = 0; str[i] != 0; i++) { 
		char *tmp = str[i]; // tmp에 str의 요소를 하나씩 저장
		while (1) {
			if (*tmp == '|') { // 저장한 것이 |인 경우
				j = i + 1;
				...
				com2[j - i - 1] = '\0'; // 문자열의 끝에 null을 넣음
				return;
			}
			if (*tmp++ == '\0') // 문자열 끝(null)을 만나면 종료
				break;
		}
